...
//  traversal.CreateBinaryTree(in_stream);
  traversal.CreateTree(in_stream);
//  traversal.ReadTree(in_stream);
  ...
  //int min_cost = traversal.DoTraversalDepthfirst();
  out_stream << "DEPTH FIRST MIN COST is " << min_cost << endl;
  //min_cost = traversal.DoTraversalBreadthfirst();
  out_stream << "BREADTH FIRST MIN COST is " << min_cost << endl;
  out_stream.flush();
